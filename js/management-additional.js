...
async function uploadAdditionalFile(type,id,def,file){
  ...
  const storage=storageMod.getStorage(appMod.getApp());
  await storageMod.uploadBytes(storageMod.ref(storage,path),file,{contentType:file.type||'application/octet-stream'});
  return path;
}
async function deleteStoredFile(path){
  if(!path||!session.uid||!String(path).startsWith(`accounts/${session.uid}/`))return;
  try{const {appMod,storageMod}=await storageApi();await storageMod.deleteObject(storageMod.ref(storageMod.getStorage(appMod.getApp()),path))}catch(e){console.warn('No se pudo borrar el archivo anterior',e)}
}
...
async function renderFor(type){const box=document.getElementById(type==='CLIENT'?'clientAdditional':'userAdditional');if(!box)return;const id=selected[type];if(!id){box.innerHTML=`Selecciona un ${type==='CLIENT'?'cliente':'usuario'}.`;return}if(!allRows.length)await loadDefinitions();const defs=defsFor(type),values=await loadValues(type,id);Object.keys(values).forEach(k=>chosen[type].add(String(k)));const selectedDefs=defs.filter(d=>chosen[type].has(String(d.id_sequence)));box.innerHTML=`<form class="additional-form" data-additional-type="${type}"><div class="additional-picker-row"><div class="additional-picker-hint">Añadir dato adicional</div><button class="additional-more" type="button" title="Seleccionar dato" aria-label="Seleccionar dato">...</button></div><div class="additional-fields">${selectedDefs.map(d=>`<div class="additional-field-row"><div class="additional-field-name">${esc(d.name)}</div><div class="additional-field-control">${controlHtml(d,values[String(d.id_sequence)])}</div></div>`).join('')}</div>${selectedDefs.length?'<div class="form-actions"><button class="primary" type="submit">Guardar</button></div>':''}<div class="status additional-status"></div></form>`;box.querySelector('.additional-more').onclick=()=>openPicker(type);box.querySelectorAll('.additional-file-open').forEach(b=>b.addEventListener('click',async()=>{try{await openStoredFile(b.dataset.filePath)}catch(err){console.error(err);const s=box.querySelector('.additional-status');if(s){s.textContent='ERROR REAL: '+(err?.message||String(err));s.className='status additional-status error'}}}));box.querySelector('form').addEventListener('submit',e=>saveAdditional(e,type,id,selectedDefs))}
